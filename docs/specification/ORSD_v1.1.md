...
- **CQ 5.2:** Which external ontology classes are linked to a given metadata:DrawingElement for cross-domain interoperability?  *(**OI-1 resolved**: the linking property `metadata:depicts` is now defined in `aec_drawing_metadata`. It asserts no domain or range, because use cases place the subject differently - a reference symbol in UC-03, a whole drawing in UC-06 - so a narrower domain would entail those subjects are of a type they are not. An inverse is not yet minted; see [#80](https://github.com/BuroHappoldMachineLearning/ADIRO/issues/80).)*
