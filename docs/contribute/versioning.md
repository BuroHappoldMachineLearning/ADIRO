...
On any PR that touches an ontology, a second workflow (`compat-diff-comment.yml`) posts/updates a **sticky comment** reporting each changed module's **prospective next version** — the minimum SemVer the accumulated unreleased changes will require at the next release cut (`compat_diff.py --markdown`). During normal editing `owl:versionInfo` stays at the last released value, so a changed module shows as `RELEASE_PENDING` (a forecast, not a warning); `INSUFFICIENT_BUMP` only fires on a release-cut PR that bumped `owl:versionInfo` by less than the change requires.
