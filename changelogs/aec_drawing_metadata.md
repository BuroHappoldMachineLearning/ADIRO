...
- **`:UnidentifiedField` sentinel** ([MLE-364](https://bhmlrnd.youtrack.cloud/issue/MLE-364), B5/MLE-377). The `skos:Concept` a `FieldAssertion` is bound to (`aprov:assertsFieldKind`) when an extracted field cannot be matched to any scheme member: the caption (`aprov:capturedCaption`) and value are kept, no promotion happens (no `aprov:mapsToFieldProperty`), and the proportion bound to it is a red-flag / coverage metric. **Deliberately NOT `skos:inScheme` `TitleblockFieldScheme`**, so it is never itself a field the pipeline searches for.
- **Content-provenance & synonym-uniqueness policy on `TitleblockFieldScheme`** — a `skos:scopeNote` records the public/private basis of the vocabulary (all labels/synonyms/examples are generic and public; project-specific keys stay in the per-project JSON overlay; per-item `dcterms:source` to follow when the anonymised dataset is public, private inventory tracked in RES-108), and a `skos:note` states the contract the JSON-vs-ADIRO consistency check ([MLE-376](https://bhmlrnd.youtrack.cloud/issue/MLE-376) B4) relies on: within the scheme a given `skos:altLabel` string, per language, denotes at most one field kind.
...
- **`DrawingSheet`'s `RevisionTable` cardinality relaxed from `exactly 1` to `min 0`.** A sheet is no longer required to contain a revision table *directly*.
  ...
  - **Classified non-breaking** per the [compatibility-diff spec](../docs/contribute/versioning/compatibility-diff-algorithm-spec.md) (`RESTRICTION_LOOSENED`), and nothing previously valid becomes invalid. Worth noting the one real cost anyway: an *entailment is lost*. You could previously infer from `?x a DrawingSheet` that `?x` contains a `RevisionTable`; you no longer can. No competency question in `docs/specification/use-cases/` was found to depend on that — checked before making the change — but a consumer relying on the inference rather than on asserted data would notice.
