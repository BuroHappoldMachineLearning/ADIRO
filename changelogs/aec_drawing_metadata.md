...
- **`depicts`** ([#77](https://github.com/BuroHappoldMachineLearning/ADIRO/issues/77); ORSD CQ 5.2, open issue OI-1) — generic object property linking something drawn to the real-world thing it graphically represents, including a term from an external vocabulary (ifcOWL, BOT, a material taxonomy). `rdfs:domain` and `rdfs:range` are deliberately left as `owl:Thing` because use cases place the subject differently (a reference symbol in UC-03, a whole drawing in UC-06); narrower named subproperties are left to the use case that needs them. The DAnO crosswalk for it lives in the optional `aec_dano_alignment` module, not here.
